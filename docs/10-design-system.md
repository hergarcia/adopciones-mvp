...
| `Stamp` | ui | `primary` `warning` `muted`; `md` `lg` | El sello (`.sello`) con su color: yerba lo verificado, mate cocido cuando le toca actuar a alguien, `--color-ink-muted` lo que se cerró. `md` en `--text-sm`, el de todos los días; `lg` en `--text-xl`, solo en la pantalla cuyo estado es el logro. Lo usan `IdentityStamp`, `PhoneNumberCard` y `NumberLostNotice`; un color nuevo se agrega acá cuando un estado lo pida. |
...
| `IdentityRequestForm` | verification | consentimiento · fotos · enviando · error | La vista de pedir la verificación de identidad: los dos pasos en la misma pantalla, sin navegar. La tirita nombra lo que pasa después —«Acepto y elijo las fotos», después «Enviar mi pedido»—, nunca «Aceptar» a secas; «Enviar» queda deshabilitada hasta tener las dos fotos procesadas. Si la cuenta bajó a sin verificar en el medio, lleva a la puerta del teléfono; si la sesión venció, a ingresar. El error del envío va arriba de la tirita. Desde 768, las dos fotos lado a lado como pares, con los huecos a la misma altura aunque la selfie lleve el ejemplo arriba; el consentimiento y la tirita siguen en `--measure` (se agregan columnas, como en `ReviewRequestView`). |
...
| `ReviewRequestView` | verification | con imágenes · propio | Un pedido: el nombre en voz de afiche, la zona, desde cuándo tiene cuenta, los rechazos de 30 días, las dos imágenes (`ReviewImage`) y la regla de qué mirar. Desde 1024, la cédula y la selfie lado a lado a la misma altura —comparar las dos caras es la decisión, y no puede depender del scroll—; debajo, los datos a la izquierda y la regla con lo que se decide a la derecha, así el botón sigue a la vista con las fotos. |
