...
| `PhoneCodeForm` | verification | quieto · verificando · error por motivo · reenviando · reenviado · esperando · número en uso | Una sola hoja con el reenvío, porque pedir otro vacía el renglón y vuelve a contar la espera y los intentos. Recibe el encabezado de la pantalla como `header`: con el número en otra cuenta, el encabezado, el renglón, el reenvío y «Corregir el número» se van, y queda `NumberInUseWays`. |
...
| `NumberInUseWays` | verification | con y sin «Seguir» | Un `h1` que nombra el problema, anunciado con `role="alert"`, y los caminos de quien encuentra su número en otra cuenta, sin nada de esa cuenta. La tirita es el próximo paso real: «Seguir» si la cuenta volvió a nivel 1 y se llegó por una acción, «Verificar otro número» si no. |
| `CancelPendingButton` | verification | quieto · cancelando | El botón `ghost` de un `<form action>`: cancelar funciona sin JavaScript. El formulario es del servidor; solo `CancelPendingSubmit`, la hoja cliente, agrega el estado ocupado. |
