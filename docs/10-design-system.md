...
- Banda: 8 px a la izquierda de una tira de papel de aviso (`Toast`, `SaveFailedNotice`), en
  `--color-primary` si salió bien y `--color-accent` si no. Es el único trazo que no mide 2 px, y
  vive una sola vez en la receta `paperStrip` de `ui/`.
...
| `ProfileForm` | profile | `editando` `guardando` `saliendo` `error` `aviso de no guardado` | Coordina los campos y el guardado, con cada error debajo de su campo y el de guardado arriba del botón. Sirve para completar y para editar; cambia el verbo, no el formulario. Con cambios sin guardar, un clic en un enlace nuestro abre `LeavingDialog` en vez de navegar: perder lo escrito tampoco se deshace. Un guardado que no llega —sin conexión, sin respuesta en 30 s o con la sesión vencida— no rompe la pantalla ni borra nada: monta `SaveFailedNotice` arriba de la tirita, y mientras siga ahí la pantalla tiene cambios sin guardar. Reintentar y guardar son el mismo toque y mandan lo que está en pantalla. |
| `LeavingDialog` | profile | — | El aviso de salir con cambios sin guardar, en `Dialog`. «Seguir editando» primero, en `primary`, porque es lo que quiere quien llegó ahí sin querer; «Salir sin guardar» en `secondary`. |
| `SaveFailedNotice` | profile | `offline` · `no_response` · `session` | El aviso de un guardado que no llegó: la misma tira de papel que el `Toast` de error (`paperStrip`, banda `error`) —borde de tinta de 2 px y la banda de ceibo a la izquierda, texto en tinta, `--space-4` de relleno—, pero quieta dentro del formulario, pegada arriba de la tirita y sin sombra ni cinta: no flota sobre nada. Un error dicho con el lenguaje del cartel y no con una caja de color. **Decisión (2026-09-26):** reemplaza el renglón sobre `--color-accent-soft` sin borde, que se leía como la alerta de cualquier sitio. Entra con `fade-in` y el texto se anuncia (`role="alert"`); en cada fallo se vuelve a montar solo el texto, para anunciarse otra vez sin llevarse el botón: si el reintento vuelve a fallar, el foco vuelve a «Reintentar». Dice qué pasó y que lo escrito sigue ahí, y ofrece «Reintentar» en `ghost`, deshabilitado mientras hay un intento en curso; con la sesión vencida, «Entrar de nuevo» es un enlace `ghost` que vuelve a la misma pantalla. En el alta el aviso dice que el nombre y la zona esperan a la vuelta (y, si había una foto elegida, que hay que elegirla de nuevo), y ese enlace sale sin `LeavingDialog`: el borrador sobrevive, así que «lo que escribiste se pierde» sería falso. Al editar sí pregunta antes. La tirita no cambia de texto. |
| `DiscardProfileDraft` | profile | — | Hoja cliente sin dibujo en «Mi perfil»: al montar borra el borrador del alta de este navegador, que a esa pantalla solo se llega con el alta terminada. |
...
| `PhoneSectionLabel` | verification | — | «Tu teléfono»: la etiqueta de la sección en `--text-sm` y `--color-ink-muted`, igual en los cuatro estados de `PhoneStatusCard` y encima de `PhoneNumberCard`, como «Tu correo» arriba. |
