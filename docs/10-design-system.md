...
| `PhoneCodeForm` | verification | quieto · verificando · error por motivo · reenviando · reenviado · esperando · número en uso | Una sola hoja con el reenvío, porque pedir otro vacía el renglón y vuelve a contar la espera y los intentos. Recibe el encabezado de la pantalla como `header`: con el número en otra cuenta, el `h1` pasa a nombrar el problema (anunciado) y el renglón, el reenvío y «Corregir el número» se van; quedan los caminos de `NumberInUseWays`. |
