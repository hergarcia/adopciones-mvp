...
| `SaveFailedNotice` | profile | `offline` · `no_response` · `session` | El aviso de un guardado que no llegó: un renglón del formulario, pegado arriba de la tirita, sobre `--color-accent-soft` con el texto en tinta (el ceibo sobre ese fondo no llega a AA), `--space-4` de relleno, sin borde, sombra ni cinta. Entra con `fade-in` y se anuncia (`role="alert"`); se vuelve a montar en cada fallo para anunciarse otra vez. Dice qué pasó y que lo escrito sigue ahí, y ofrece «Reintentar» en `ghost`, deshabilitado mientras hay un intento en curso; con la sesión vencida, «Entrar de nuevo» es un enlace `ghost` que vuelve a la misma pantalla. Es lo único con fondo de color en la pantalla; la tirita no cambia de texto. |
