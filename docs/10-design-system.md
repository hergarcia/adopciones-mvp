...
| `IdentityConsent` | verification | a la vista · plegado | Qué se pide y qué se hace con las imágenes (FR-004), en un bloque sobre `--color-surface`. Aceptado, se pliega a una línea con el tilde en yerba y «Leer de nuevo»: un `details` nativo, sin JavaScript, para que las fotos queden a la vista sin perder el texto. |
...
| `IdentityStamp` | verification | los cinco estados | El sello del pedido. Yerba solo en «Nivel 2»; mate cocido en «En revisión», porque le toca actuar a alguien; `--color-ink-muted` en lo que se cerró sin nivel 2. Nunca el acento: un rechazo no es un error. |
...
| `WithdrawRequestDialog` | verification | confirmando · retirando · error | Retirar el pedido, que es irreversible (las imágenes se borran): «Retirar mi pedido» en `ghost-danger` abre el `Dialog` con «Retirar mi pedido» en `danger` y «Seguir esperando». Controlado con `open`, como `DeleteAccountDialog`; el error queda adentro. |
...
| `ReviewWatcher` | verification | abierto · resuelto · vencido · ya no está | Pregunta cada 10 s si el pedido sigue abierto; si no, reemplaza la pantalla por un `EmptyState` que dice qué pasó y «Volver a la lista», y las imágenes se desmontan. Da el estado compartido de las imágenes a `ReviewImage` y `ReviewDecision`. |
