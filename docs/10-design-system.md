...
| `ResendFromLinkButton` | auth | `loading` `error` | Lo mismo desde «El enlace no sirve», pero con el id del enlace: el servidor resuelve la dirección y la pantalla nunca la conoce. |
...
| `ZoneFields` | profile | — | Departamento de lista cerrada y localidad. **La etiqueta del segundo cambia**: «Barrio» en Montevideo, «Localidad» en los otros dieciocho. |
...
| `ProfileFields` | profile | — | Los cuatro campos del perfil. Separado de `ProfileForm` porque una cosa es qué se pide y otra qué pasa al guardar. |
| `ProfileForm` | profile | `editando` `guardando` `error` | Coordina los campos y el guardado, con cada error debajo de su campo y el de guardado arriba del botón. Sirve para completar y para editar; cambia el verbo, no el formulario. |
| `ProfileSummary` | profile | con foto / sin foto; rescatista | El perfil en lectura. El sello de rescatista va en `--color-primary`: es confianza, no un estado de proceso. |
