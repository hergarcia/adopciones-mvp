...
| `GateNotice` | verification | `publish` · `apply` | El `h1` y la frase del motivo de la acción que se tocó. Solo el encabezado: la salida la pone la pantalla. |
| `PhoneStatusCard` | verification | verificado · sin teléfono · a medias · cambio a medias | La sección de teléfono de «Mi perfil». Sin teléfono es el paso pendiente y no un hueco; no es un `EmptyState`, que centra y lleva ilustración. Todo en `secondary` o `ghost`: la tirita de la pantalla sigue siendo «Editar mi perfil». |
| `VerifiedPhone` | verification | — | El número verificado, en una `Card`, con un **sello** «Verificado» en `--color-primary` y "Nivel 1 desde…" en texto. Es un sello porque es un estado que cambia; la chapita de cada nivel es la de `VerificationBadge` y llega con la historia #12. |
| `PendingPhoneNotice` | verification | primera verificación · cambio | El número a medias con un sello «Sin confirmar» en `--color-warning` (le toca actuar a alguien) y "tu cuenta está sin verificar"; en un cambio, "si cancelás, vuelve el…". El número anterior no se muestra como verificado mientras la cuenta no lo está. |
...
| `PhoneCodeForm` | verification | quieto · verificando · error por motivo · reenviando · reenviado · esperando | **Un solo `input`** con `autocomplete="one-time-code"` a `--text-2xl` con `tabular-nums`, sin espaciado extra: los seis cuadraditos de cualquier kit rompen el pegado y la sugerencia del teléfono. Una sola hoja con el reenvío, porque pedir otro vacía el renglón y vuelve a contar la espera y los intentos. |
| `NumberInUseWays` | verification | con y sin «Seguir» | Los caminos de quien encuentra su número en otra cuenta, sin nada de esa cuenta. |
